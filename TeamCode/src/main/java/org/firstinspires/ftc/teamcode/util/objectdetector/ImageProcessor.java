...
public class ImageProcessor {
    ...
    public static boolean isGreenWithMinSaturation(HSV hsv) {
        return hsv.s > MIN_SATURATION && isGreen(hsv);
    }

    public static boolean isPinkWithMinSaturation(HSV hsv) {
        return hsv.s > MIN_SATURATION && isPurple(hsv);
    }
    ...
    public static boolean isGreen(HSV hsv) {
        return hsv.h > 100 && hsv.h < 140;
    }

    public static boolean isPurple(HSV hsv) {
        return hsv.h > 181 && hsv.h < 241;
    }

    public static boolean isWhite(HSV hsv) {
        return hsv.h < 179 && hsv.h > 145 && hsv.s > 30;
    }

    // For pixel that is far, color sensor has a hard time to differentiate white form purple.
    public static boolean couldBeWhiteOrPurple(HSV hsv) {
        return hsv.h >= 179 && hsv.h <= 181;
    }
